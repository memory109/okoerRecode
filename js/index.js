window.onload = function(){
	...
	function time(){
		...
		caro.style.backgroundImage = "url(img/carousel-"+ num +".jpg)";
	...
	for(var i = 0;i < liArr.length;i++){
		...
		liArr[i].onclick = function(){
			...
			// 轮播图片
			caro.style.backgroundImage = "url(img/carousel-"+ (index + 1)+".jpg)";
			// 记录下标
			num = index;
	...
	array[0].onclick = function(){
		...
		if(arrayIndex >= 0){
			...
			caro.style.backgroundImage = "url(img/carousel-"+ (arrayIndex + 1)+".jpg)";
			num = arrayIndex;
		}else{
			changeClass(liArr[4],"change");
			caro.style.backgroundImage = "url(img/carousel-5.jpg)";
			num = 4;
		}	
		timeId  = setInterval(time,4000);	
	...
	array[1].onclick = function(){
		...
		if(arrayIndex < liArr.length){
			...
			caro.style.backgroundImage = "url(img/carousel-1.jpg)";
			num = 0;
		}	
		timeId  = setInterval(time,4000);	
	}
	...
	//报告内容区
	var repo1 = document.getElementsByName("rePo-1");
	var repo2 = document.getElementsByName("repoDiv2");

	for(var i = 0;i < repo1.length;i++){
		repo1[i].setAttribute("repoInd",i);
	}
	for(var i = 0;i < repo1.length;i++){
		repo1[i].onmouseover = function(){
			var index = parseInt(this.getAttribute("repoInd"));
			repo2[index].style.display = "block";
		}
		repo1[i].onmouseout = function(){
			var index = parseInt(this.getAttribute("repoInd"));
			repo2[index].style.display = "none";
		}
	}
